...
class ModalWindow{
	...
	public function modal_preview( $content, $element ){
		...
		$content = '<# if ( settings.show_in_modal ) {
			...
			#><button class="frontend-admin-edit-button edit-button" onClick="openModal(\'{{id}}' .get_the_ID(). '\')" >
	...
	public function before_element_render( $settings, $wg_id  ){
		if( ! isset( $settings['show_in_modal'] ) || ! $settings['show_in_modal'] ){
			...
			$before = '<button class="frontend-admin-edit-button edit-button" onClick="openModal(\'' .$modal_num. '\')" >'; 
	...
	public function modal_controls( $element, $args ) { 
		...
		$default_text = __( 'Click Here', FEA_NS );
		...
		$element->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .frontend-admin-edit-button',
			]
		);
		...
		$element->add_responsive_control(
			...
			[
				...
				'size_units' => ['px', '%', 'em'],
				'selectors' => [
					'{{WRAPPER}} .frontend-admin-edit-button' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);		
		...
		$element->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .frontend-admin-edit-button' => 'fill: {{VALUE}}; color: {{VALUE}};',
				],
			]
		);
		...
		$element->add_control(
			...
			[
				...
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .frontend-admin-edit-button' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$element->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .frontend-admin-edit-button' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
