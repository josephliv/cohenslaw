...
class ActionProduct extends ActionBase {
	...
	public function get_default_fields( $form ){
		$default_fields = array(
			'product_title', 'product_description', 'product_short_description', 'main_image', 'product_images', 'product_status',			
		);
...
/* 	public function create_product( $args ){	
	...
	public function run( $form, $step = false ){	
		...
		if( $product_to_edit['ID'] == 0 ){
			...
			update_metadata( 'post', $product_id, 'admin_form_source', $element );
		}else{
			wp_update_post( $product_to_edit );
			update_metadata( 'post', $product_id, 'admin_form_edited', $element );
		}
