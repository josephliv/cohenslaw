		...
		<thead>
			<tr>
				...
				<?php foreach( $sub_fields as $sub_field ): 
					$sub_field = acf_frontend_parse_args( $sub_field,
						array(
							'prefix' => '',
							'type' => '',
							'required' => 0,
	...
	<tbody>
		<?php foreach( $value as $i => $row ): 
			...
			<tr class="<?php echo $css_classes ?>" data-id="<?php echo esc_attr( $id ); ?>">
				...
				<?php if( $show_order ): 
				$td_class = 'acf-row-handle';
				if( empty( $field['maintain_order'] ) ) $td_class .= '  order';
				if( $show_order == 'ids' ) $td_class .= '  ids'; ?>
					<td class="<?php echo $td_class; ?>" title="<?php _e('Drag to reorder',FEA_NS); ?>">
                        <?php if( $field['collapsed'] ): ?>
						<a class="acf-icon -collapse small" href="#" data-event="collapse-row" title="<?php _e('Click to toggle',FEA_NS); ?>"></a>
                        <?php endif; ?>
						<span><?php
						if( $show_order == 'ids' ){
							echo $row['id'];
