...
class acf_field_related_terms extends acf_field {
	...
	*/
	...
	function initialize() {
		...
		// actions
		add_filter( 'acf/prepare_field/type=taxonomy',  [ $this, 'load_taxonomy_field'] );
	...
	*/
	...
	function get_ajax_query( $options = array() ) {
		...
		foreach( $terms as $term ) {
			...
			$results[] = array(
				'id'	=> $term->term_id,
				'text'	=> $this->get_term_title( $term, $options['taxonomy'], $options['post_id'] )
			);
	...
	*/
	...
	function get_term_title( $term, $taxonomy, $post_id = 0 ) {
...
<div <?php acf_esc_attr_e($div); ?>>
	...
	if( $field['add_term'] ): ?>
		<div style="margin-top:20px">
			<a href="#" class="acf-button button add-new-term" data-name="add"><?php echo esc_attr($taxonomy->labels->add_new_item); ?></a>
		</div>
		<?php endif; ?>
</div><?php
		...
		if( !empty($field['value']) ) {
			...
			if( !empty($terms) ) {
				...
				foreach( array_keys($terms) as $i ) {
					...
					// append to choices
					$field['choices'][ $term->term_id ] = $this->get_term_title( $term, $field['taxonomy'] );
...
<div class="categorychecklist-holder">
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		// add_term
		acf_render_field_setting( $field, array(
			'label'			=> __('Create Terms',FEA_NS),
			'instructions'	=> __('Allow new terms to be created whilst editing',FEA_NS),
			'name'			=> 'add_term',
			'type'			=> 'true_false',
			'ui'			=> 1,
		));
