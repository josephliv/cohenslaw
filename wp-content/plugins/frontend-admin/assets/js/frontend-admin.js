(function($){
	...
	$('body').on('click', 'button.edit-password', function(){
		$(this).addClass('frontend-admin-hidden').parents('.acf-field-user-password').removeClass('edit_password').addClass('editing_password').siblings('.acf-field-user-password-confirm').removeClass('frontend-admin-hidden');
		$(this).after('<input type="hidden" name="edit_user_password" value="1"/>');
		$(this).siblings('.pass-strength-result').removeClass('frontend-admin-hidden')
	});
	
	$('body').on('click', 'button.cancel-edit', function(){
		$(this).siblings('button.edit-password').removeClass('frontend-admin-hidden').parents('.acf-field-user-password').addClass('edit_password').removeClass('editing_password').siblings('.acf-field-user-password-confirm').addClass('frontend-admin-hidden');
		$(this).parents('acf-input-wrap').siblings('acf-notice');
		$(this).siblings('input[name=edit_user_password]').remove();
		$(this).siblings('.pass-strength-result').addClass('frontend-admin-hidden');
	});
	...
	$(function(){
		...
		if( ! forceEdit ){
			$('.acf-field-user-password-confirm').removeClass( 'frontend-admin-hidden' );
		}
	});
	...
	acf.getForm = function( $clicked ){			
		var $form_args;
		if($clicked.data('name')=='admin_form'){
			var $el = $clicked;
	...
	$('body').on('click','.frontend-form button[type=submit]',function(e){
		e.preventDefault();
					...
					if( data.redirect ){
						...
						//modal window ajax form
							...
							if(data.reloaded){
								...
								$form.replaceWith(data.reloaded);
								$form=$('#' + formID);
	...
	$.each( dynamicValueFields, function( key, value ){
		...
		if( fieldSources.length > 0 ){
			...
			$.each( fieldSources, function( index, fieldName ){
				...
				var sourceInput = frontend_admin_get_field_element(fieldData[0], false);
				inputValue = frontend_admin_get_field_input_value(inputValue, fieldData, sourceInput); 
				var sourceInput = frontend_admin_get_field_element(fieldData[0], true);  
				sourceInput.on('input', function(){
					var returnValue = fieldDynamicValue;
					$.each( fieldSources, function( index, fieldName ){
					var fieldData = acf_frontend_get_field_data(fieldName);               
					var sourceInput = frontend_admin_get_field_element(fieldData[0], false);
					returnValue = frontend_admin_get_field_input_value(returnValue, fieldData, sourceInput);
					});
			...
			function frontend_admin_get_field_input_value(returnValue, fieldData, sourceInput){
				var shortcode = '['+fieldData[0]+']';
				if( sourceInput.val() != '' ){
				var display = sourceInput.val();
				if(fieldData[1] == 'text'){
					var display = frontend_admin_get_field_text(fieldData[0]);
				}
			...
			function frontend_admin_get_field_element(fieldName, all){
			...
			function frontend_admin_get_field_text(fieldName){
				var sourceField = fieldElement.siblings('div[data-name=' + fieldName + ']');
				if(sourceField.data('type') == 'radio'){
	...
	var imageFields = ['featured_image','main_image','site_logo','upload_image'];
